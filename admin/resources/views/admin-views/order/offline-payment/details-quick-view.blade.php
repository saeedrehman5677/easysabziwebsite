        ...
        <div class="text-center">
            ...
            <div class="btn--container justify-content-end my-2 mx-3">
                @if($order->offline_payment?->status == 0)
                    <a type="reset" class="btn btn--reset verify-offline-payment-button" data-id="{{ $order['id'] }}" data-status="2">{{ translate('Payment_Did_Not_Received') }}</a>
                @endif
                <a type="submit" class="btn btn--primary verify-offline-payment-button"  data-id="{{ $order['id'] }}" data-status="1" >{{ translate('Yes') }}, {{ translate('Payment_Received') }}</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $('.verify-offline-payment-button').on('click', function() {
        var orderId = $(this).data('id');
        var orderStatus = $(this).data('status');
        verify_offline_payment(orderId, orderStatus);
    });
